...
public class Bully {
    ...
    public static void up(int id) {
        if (processes[id])
            System.out.println("P" + id + " is already up and running\n");
        else {
            processes[id] = true;
    ...
    public static void displayProcesses() {
        for (int i = 1; i <= n; i++) {
            if (processes[i])
                System.out.println("P" + i + ": up");
            else
                System.out.println("P" + i + ": down");
        }
        ...
        if (!higherAlive) {
            ...
            for (int i = 1; i <= n; i++) {
                if (i != coordinator && processes[i])
                    System.out.println("P" + coordinator + " -> P" + i + ": COORDINATOR\n");
            }
        }
    }
...
// 24 25 26 Perform Leader Election using Bully Algorithm. Also print the
// messages
